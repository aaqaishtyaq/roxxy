...
func NewFileLogger(path string) (*Logger, error) {
	...
	return NewWriteLogger(file), nil
}
...
func NewSyslogLogger() (*Logger, error) {
	...
	return NewWriteLogger(writer), nil
}
...
func NewStdoutLogger() (*Logger, error) {
	return NewWriteLogger(syncCloser{os.Stdout}), nil
}

func NewWriteLogger(writer io.WriteCloser) *Logger {
	log := Logger{
	...
	for el := range l.logChannel {
		...
		if el.Err != nil {
			if len(el.Err.RawMessaeges) > 0 {
				fmt.Fprintf(l.writer, el.Err.RawMessaeges...)
				continue
			}
