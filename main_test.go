...
func (s *S) TestServeHTTPStressAllLeakDetector(c *check.C) {
	...
	checkLeaksEnabled := os.Getenv("ROXXY_CHECK_LEAKS") != ""
	...
	for i := range frontends {
		...
		for j := 0; j < ratio; j++ {
			err := s.redis.RPush(ctx, "frontend:"+frontend, servers[(i*ratio)+j].URL).Err()
			c.Assert(err, check.IsNil)
		}
		if i > nFrontends/2 {
			// Add invalid backends forcing errors on half of the frontends
			err := s.redis.RPush(ctx, "frontend:"+frontend, "http://127.0.0.1:32412", "http://127.0.0.1:32413").Err()
			c.Assert(err, check.IsNil)
		}
	}
	nProffs := 4
	...
	routesBE, err := backend.NewRedisBackend(ctx, opts, opts)
	c.Assert(err, check.IsNil)
