...
func (s *S) TestRedisCertificateFound(c *check.C) {
	result, err := s.redisClient.HMSet(ctx, "tls:certificate.com", map[string]interface{}{
...
func (s *S) TestRedisCertificateCached(c *check.C) {
	result, err := s.redisClient.HMSet(ctx, "tls:certificate-cached.com", map[string]interface{}{
		"certificate": rsaCertPEM,
		"key":         rsaKeyPEM,
	}).Result()
	c.Assert(err, check.IsNil)
	...
	deleted, err := s.redisClient.Del(ctx, "tls:certificate-cached.com").Result()
	c.Assert(err, check.IsNil)
